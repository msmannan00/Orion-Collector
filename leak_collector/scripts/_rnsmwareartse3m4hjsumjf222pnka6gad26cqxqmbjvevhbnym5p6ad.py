    ...
    def parse_leak_data(self, page: Page):
        try:
            ...
            for card in cards:
                ...
                ref_html = None
                if not is_crawled and list(weblink).__len__()>0:
                    ref_html = helper_method.extract_refhtml(list(weblink)[0])
                    if ref_html:
                        self.invoke_db(REDIS_COMMANDS.S_SET_BOOL, CUSTOM_SCRIPT_REDIS_KEYS.URL_PARSED.value + list(weblink)[0], True)
                ...
                card_data = leak_model(
                    ...
                    m_dumplink=list(cleaned_dumplink),
